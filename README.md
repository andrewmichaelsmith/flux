...
| Fake LLM-API endpoint | Ollama / OpenAI / Anthropic-proxy JSON on `/v1/models`, `/v1/chat/completions`, `/anthropic/v1/messages`, `/api/chat`, … ; logs model + auth header + prompt prefix — [docs](./docs/fake-llm-api.md) | no |
...
| PHP/body-RCE responders | Body-driven exploit responders for PHPUnit `eval-stdin.php`, PHP-CGI `auto_prepend_file=php://input`, and Apache CGI path-traversal `/bin/sh`; logs request-body payloads and decoded base64 command hints — [docs](./docs/cmd-injection.md) | no |
...
| Production .env and its sibling rotation/backup/per-env variants | `/.env.{production,prod,live,local,dev,development[.local],test[.local],staging,example[.local],ci,save,private,docker,override}`, `/.env2`, `/.env_{bak,old,orig,priv,example}`, `/.environ`, plus webroot-prefix `/{mailer,opt,srv,var/www,app}/.env` | `aws` | `env-production` |
