...
| Fake webshell | Plausible File Manager on known `*.php` shell probe paths — includes the systematic shell-hitlist bundle, the `/style.php` × WP-directory family, and shell-shaped filenames from a coordinated multi-region webshell-scanner cohort (`/this_is_a_new_hello_world.php`, `/wp-content/{admin,index,themes/index}.php`, `/wp{,-load}.php`, `/w.php`, `/file.php`, `/2.php`, `/82.php`, `/goods.php`, `/bless.php`, `/simple.php`, `/atomlib.php`) plus shell-jacking regex families (`/.well-known/<name>.php`, `/.trash<N>/*`, `/.tmb/`, `/.dj/`, `/.alf/`, …); simulates `id` / `whoami` / `uname -a` / `cat /etc/passwd` on follow-up commands — [docs](./docs/fake-webshell.md) | no |
...
| --- | --- | --- | --- |
| AWS credentials file (INI) | `/.aws/credentials` plus `.bak` / `.old` editor-leftover variants, home-dir prefix variants (`/root/.aws/credentials`, `/home/{ubuntu,ec2-user,admin,app,node,deploy,www-data}/.aws/credentials`), and the bare webroot-dropped `/credentials` | `aws` | `aws-credentials-file` |
