...
| `wpRestPrefixlessRoute` | string | Present only when the path names the WordPress REST `wp/v2` namespace with no base segment (`/wp/v2/posts/999999`, `/blog/wp/v2/users`) — the spelling tooling emits after mis-joining a route key onto a base URL that already ends in `/wp-json`. The value is the canonical route asked for. Stamped ahead of dispatch and **purely descriptive**: the request still falls through to the same 404, because a real install serves REST under exactly one `rest_url_prefix` and answering both spellings would be a fleet-wide tell. See [docs/wp-rest-route-alias.md](./docs/wp-rest-route-alias.md). |
