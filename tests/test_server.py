...
def test_format_env_payload_no_honeypot_tell_in_output():
    ...
    # mention of "honeypot", "canary", "tracebit", or "autogenerated" in the
    # human-readable header burns the entire trap. Credential-key names
    # (SSH_CANARY_*, TRACEBIT_CANARY_ERROR) are intentionally out of scope
    # here — this test only guards the non-credential header lines.
    response = {
        "aws": {
            "awsAccessKeyId": "AKIAEXAMPLE",
            "awsSecretAccessKey": "supersecret",
    ...
    payload = tbenv.format_env_payload(response)
    header_lines = [ln for ln in payload.splitlines() if ln.startswith("#")]
    joined = "\n".join(header_lines).lower()
    for tell in ("honeypot", "autogenerated", "canary", "tracebit"):
        assert tell not in joined, f"header leaks '{tell}': {header_lines!r}"
