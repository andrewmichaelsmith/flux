...
flux makes **no outbound request** on any branch. A target we do not
emulate is logged and refused, never fetched — the trap must never
behave like a working open proxy.

Every line carries `ssrfParam`, `ssrfTarget`, `ssrfHost`,
`ssrfTargetPath` and `ssrfCloud` on top of the usual metadata fields,
plus the `imdsKind` / `imdsRole` pair on the AWS branch.
