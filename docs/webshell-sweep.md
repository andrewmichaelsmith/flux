...
Serves the fake webshell on *any* unclaimed root-level `*.php` name, but only
to a source that has already demonstrated sweep behaviour.

## Matching

| Property | Value |
| --- | --- |
| Shape | `^/[A-Za-z0-9][A-Za-z0-9._-]{0,63}\.php$` — one root-level segment |
| Priority | Last in dispatch, after every exact-path trap has had its turn |
| Never matched | `/index.php`, `/home.php`, `/main.php`; anything already in the curated `WEBSHELL_PATHS` list; anything another trap claimed |
...
dictionary of root-level `.php` filenames. The curated list this trap started
